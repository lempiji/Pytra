...
def _resolve_d_output_path(input_path: Path, args: argparse.Namespace) -> Path:
    if args.output != "":
        return Path(args.output)
    out_dir = Path(args.output_dir) if args.output_dir else Path(DEFAULT_OUTPUT_DIR)
    stem = input_path.stem
    if stem == "":
        stem = "output"
    return out_dir / f"{stem}.d"


def _transpile_d(input_path: Path, args: argparse.Namespace, passthrough: list[str]) -> int:
    output_path = _resolve_d_output_path(input_path, args)
    if output_path.exists() and output_path.is_dir():
        print(f"error: output path is a directory: {output_path}", file=sys.stderr)
        return 1
    output_path.parent.mkdir(parents=True, exist_ok=True)
    cmd = [PYTHON, str(PY2D), str(input_path), "--output", str(output_path)]
    cmd.extend(passthrough)
    return _run(cmd, cwd=ROOT)
...
def _parse(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    ap.add_argument("--target", default="cpp", choices=["cpp", "rs", "scala", "d"], help="target language")
...
def main(argv: list[str]) -> int:
    ...
    if args.target == "d":
        return _transpile_d(input_path, args, passthrough_args)
